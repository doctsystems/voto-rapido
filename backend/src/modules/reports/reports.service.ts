...
export class ReportsService {
  ...
  async exportExcel(currentUser: any, res: Response) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=voto-rapido-${Date.now()}.xlsx`);
  ...
  async exportPdf(currentUser: any, res: Response) {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=voto-rapido-${Date.now()}.pdf`);

    const doc = new PDFKit({ margin: 50 });
    doc.pipe(res);

    // Header
    doc.fontSize(20).fillColor('#1B4F72').text('VotoRápido - Reporte de Votación', { align: 'center' });
    doc.fontSize(10).fillColor('#666').text(`Generado: ${new Date().toLocaleString('es-EC')}`, { align: 'center' });
    doc.moveDown(2);

    // Table of reports
    reports.forEach((r, i) => {
      if (i > 0) doc.moveDown();
      doc.fontSize(12).fillColor('#1B4F72').text(`Mesa ${r.table?.tableNumber} - ${r.table?.school?.recintoElectoral ?? ''}`);
      ...
      Object.entries(byType).forEach(([typeName, entries]) => {
        ...
        entries.forEach(e => {
          doc.fontSize(9).fillColor('#333').text(`    ${e.party?.acronym} - ${e.party?.name}: ${e.votes} votos`);
        });
      });
    });

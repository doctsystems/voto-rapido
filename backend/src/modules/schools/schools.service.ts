...
export class CreateSchoolDto {
  @IsString() recintoElectoral: string;
...
export class SchoolsService {
  ...
  async create(dto: CreateSchoolDto, actorId: string) {
    const existing = await this.repo.findOne({ where: { recintoElectoral: dto.recintoElectoral } });
    if (existing) throw new ConflictException('Ya existe un recinto con ese nombre');
    const school = this.repo.create({ ...dto, createdBy: actorId, updatedBy: actorId });
  ...
  async remove(id: string, actorId: string) {
    ...
    if (school.tables?.length > 0) {
      throw new BadRequestException(`No se puede eliminar: tiene ${school.tables.length} mesa(s) asignada(s)`);
    }
    await this.repo.update(id, { deletedBy: actorId });
    await this.repo.softDelete(id);
    return { message: `Recinto "${school.recintoElectoral}" eliminado` };
  }

  findByMunicipio(municipio: string) {
    return this.repo.find({ where: { municipio }, relations: ['tables'], order: { recintoElectoral: 'ASC' } });
  }
}

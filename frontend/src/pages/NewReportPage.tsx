...
export default function NewReportPage() {
  ...
  const buildPayload = () => {
    ...
    for (const [etId, pv] of Object.entries(votes)) {
      ...
      if (!addedExtras.has(etId)) {
        ...
        extras.push({ electionTypeId: etId, nullVotes: nulls[etId] || 0, blankVotes: blanks[etId] || 0 });
      }
    }
    ...
    return { entries, extras, notes, ...(isJefeRecinto && selectedTableId ? { tableId: selectedTableId } : {}) };
  };
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      ...
      if (payload.entries.length === 0) throw new Error('Debes ingresar al menos un voto');
  ...
  const overLimitEt = sections.filter(([etId]) => tableVoters && perTypeTotal(etId) > tableVoters);
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        {isEditing && (
          <span className="inline-flex items-center gap-1.5 mt-2 text-xs bg-warning/10 border border-warning/20 text-warning px-3 py-1 rounded-full">
            ✏️ Editando reporte {existingReport.status === 'DRAFT' ? 'borrador' : 'enviado'} — se restablecerá a borrador
          </span>
        )}
      </div>
      ...
      {isJefeRecinto && (
        <div className="rounded-xl border border-stroke bg-white shadow-card p-5 mb-5">
          ...
          {(recintoTables as any[]).length === 0 ? (
            <p className="text-body text-sm">No hay mesas registradas en su recinto.</p>
          ) : (
            <select
            ...
            >
              ...
              {(recintoTables as any[]).map((t: any) => (
                <option key={t.id} value={t.id}>
                  Mesa {t.tableNumber}
                  {t.totalVoters ? ` · ${t.totalVoters.toLocaleString()} votantes` : ''}
          ...
          {selectedTableId && existingReport && (
            <p className="mt-2 text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5">
              ✏️ Esta mesa ya tiene un reporte {existingReport.status === 'DRAFT' ? 'borrador' : 'enviado'} — se editará
            </p>
          )}
        </div>
      )}
          ...
          {sections.map(([etId, section]) => {
            ...
            return (
              <div key={etId} className="rounded-2xl bg-white border border-black/[.06] shadow-[0_2px_16px_rgba(0,0,0,.06)] overflow-hidden">
                {/* Section header */}
                <div className={`flex items-center justify-between px-6 py-3 border-b border-stroke ${overLimit ? 'bg-meta-1/10 border-meta-1/30' : 'bg-whiter'}`}>
                  <div className="flex items-center gap-3">
                    <div className="w-1.5 h-5 rounded-full bg-primary" />
                    <h3 className="font-semibold text-black">{section.etName}</h3>
                    <span className="text-xs text-body">({section.rows.length} partido{section.rows.length > 1 ? 's' : ''})</span>
                  </div>
                  <div className="flex items-center gap-4 text-xs font-mono">
                    <span className="text-body">Válidos: <span className="font-bold text-meta-3">{validVotes.toLocaleString()}</span></span>
                    <span className="text-body">Nulos: <span className="font-bold text-meta-8">{(nulls[etId] || 0).toLocaleString()}</span></span>
                    <span className="text-body">Blancos: <span className="font-bold text-bodydark">{(blanks[etId] || 0).toLocaleString()}</span></span>
                    {tableVoters && (
                      <span className={`font-bold ${overLimit ? 'text-meta-1' : 'text-black'}`}>
                        Total: {typeTotal.toLocaleString()} / {tableVoters.toLocaleString()}
                        {overLimit && ' ⚠'}
                      </span>
                    )}
                  </div>
                ...
                <div className="p-5">
                  ...
                  <div className="space-y-3 mb-5">
                    {section.rows.map((row: any) => (
                      <div key={row.partyId} className="flex items-center gap-4 flex-wrap">
                        <div className="flex items-center gap-2 w-52 min-w-0 flex-shrink-0">
                          <div className="w-3 h-3 rounded-md flex-shrink-0" style={{ backgroundColor: row.partyColor || '#999' }} />
                          <span className="text-sm font-semibold text-black">{row.partyAcronym}</span>
                          <span className="text-xs text-body truncate">{row.candidateName || row.partyName}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <button type="button"
                            onClick={() => setVote(etId, row.partyId, (votes[etId]?.[row.partyId] || 0) - 1)}
                            className="w-8 h-8 rounded border border-stroke flex items-center justify-center text-body hover:bg-stroke transition-colors select-none text-lg leading-none">−</button>
                  ...
                  <div className="border-t border-stroke pt-4 grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-body mb-1.5 uppercase tracking-wide">Votos Nulos</label>
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={() => setNull(etId, (nulls[etId] || 0) - 1)}
                          className="w-8 h-8 rounded border border-stroke flex items-center justify-center text-body hover:bg-stroke transition-colors text-lg leading-none select-none">−</button>
                        <input type="number" min={0} value={nulls[etId] || ''}
                          onChange={e => setNull(etId, parseInt(e.target.value) || 0)}
                          className="w-20 text-center font-mono text-sm border border-stroke rounded py-1.5 outline-none focus:border-primary transition-colors"
                          placeholder="0" />
                        <button type="button" onClick={() => setNull(etId, (nulls[etId] || 0) + 1)}
                          className="w-8 h-8 rounded border border-stroke flex items-center justify-center text-body hover:bg-stroke transition-colors text-lg leading-none select-none">+</button>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-body mb-1.5 uppercase tracking-wide">Votos en Blanco</label>
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={() => setBlank(etId, (blanks[etId] || 0) - 1)}
                          className="w-8 h-8 rounded border border-stroke flex items-center justify-center text-body hover:bg-stroke transition-colors text-lg leading-none select-none">−</button>
                        <input type="number" min={0} value={blanks[etId] || ''}
                          onChange={e => setBlank(etId, parseInt(e.target.value) || 0)}
                          className="w-20 text-center font-mono text-sm border border-stroke rounded py-1.5 outline-none focus:border-primary transition-colors"
                          placeholder="0" />
                        <button type="button" onClick={() => setBlank(etId, (blanks[etId] || 0) + 1)}
                          className="w-8 h-8 rounded border border-stroke flex items-center justify-center text-body hover:bg-stroke transition-colors text-lg leading-none select-none">+</button>
      ...
      <div className="rounded-2xl bg-white border border-black/[.06] shadow-[0_2px_16px_rgba(0,0,0,.06)] p-5 mt-5">
        <label className="block text-sm font-semibold text-black mb-2">Notas / Observaciones (opcional)</label>

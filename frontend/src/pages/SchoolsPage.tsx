  ...
  return (
    <div>
      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-black">Recintos Electorales</h1>
          <p className="text-sm text-body mt-0.5">Gestión de recintos donde se instalan las mesas de votación</p>
        </div>
            ...
            <thead className="bg-whiten border-b border-stroke">
              <tr>
                {['Código', 'Recinto Electoral', 'Municipio', 'Circ.', 'Mesas', 'Estado', ''].map(h => (
                  <th key={h} className="text-left px-4 py-3 font-semibold text-body">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="">
              {(schools as any[]).map((s: any) => (
                <tr key={s.id} className="hover:bg-whiten transition-colors">
                  <td className="px-4 py-3 font-mono text-xs bg-whiten text-slate-500">{s.codigoRecinto || '—'}</td>
                  <td className="px-4 py-3 font-medium text-black">{s.recintoElectoral}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${s.isActive ? 'bg-green-100 text-green-700' : 'bg-meta-2 text-slate-500'}`}>
                      {s.isActive ? 'Activo' : 'Inactivo'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(s)} className="text-primary hover:text-primary-700 text-xs font-semibold">Editar</button>
                      <button
                        onClick={() => { if (confirm(`¿Eliminar "${s.recintoElectoral}"?`)) deleteMutation.mutate(s.id); }}
                        className="text-meta-1 hover:text-red-700 text-xs font-semibold">Eliminar</button>
                    </div>
                  </td>
                </tr>
              ))}
              {(schools as any[]).length === 0 && (
                <tr><td colSpan={7} className="px-4 py-10 text-center text-bodydark">No se encontraron recintos.</td></tr>
              )}
            </tbody>
          </table>
        </div>
